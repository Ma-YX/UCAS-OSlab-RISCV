...
void strServer(void)
{
    ...
    for (;;)
    {
        blockedCount += mbox_recv(mq.id, &header, sizeof(struct MsgHeader));
        blockedCount += mbox_recv(mq.id, msgBuffer, header.length);
        ...
        if (clientInitReq(msgBuffer, header.length)) {
            mbox_send(posmq.id, &clientPos, sizeof(int));
            ++clientPos;
        }
...
int clientSendMsg(mailbox_t *mq, const char* content, int length)
{
    ...
    return mbox_send(mq->id, msgBuffer, length + sizeof(struct MsgHeader));
}
...
void strGenerator(void)
{
    ...
    int position = 0;
    mbox_recv(posmq.id, &position, sizeof(int));
    int blocked = 0;
    int64_t bytes = 0;
