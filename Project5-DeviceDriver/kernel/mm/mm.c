...
uintptr_t alloc_page_helper_k(uintptr_t va, uintptr_t pgdir, pcb_t *pcb)
{
    // TODO:
    uint64_t address = pcb;
    uint64_t vpn2 = (va << 25) >> 55;
    ...
    uint64_t pgdir2_ppn;
    uint64_t pgdir3_ppn;
    uint64_t mask_12 =_PAGE_PRESENT | _PAGE_USER  | _PAGE_ACCESSED | _PAGE_DIRTY;
    uint64_t mask_3 =_PAGE_PRESENT |_PAGE_USER | _PAGE_READ | _PAGE_WRITE | _PAGE_EXEC | _PAGE_ACCESSED | _PAGE_DIRTY; 
    uint64_t valid1, valid2, valid3;
    valid1 = pgdir_entry & 0x1;
    if (valid1 == 0){
        second_pgdir = allocPage(pcb);
