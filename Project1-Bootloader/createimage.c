...
static void create_image(int nfiles, char *files[])
{
    ...
    while (nfiles-- > 0) {
        ...
        fclose(fp);
        files++;
        if (file_num == 1) // if file is kernel 1
        {
            write_os_size(nbytes - before_nbytes, img, file_num);
        }
        if (file_num == 2) // if file is kernel 2
        {
            write_os_size(nbytes - before_nbytes, img, file_num);
        }
        file_num++;
        before_nbytes = nbytes; // bytes that files before this file have written
    }
    fclose(img);
}
                          ...
                          FILE * img, int *nbytes, int *first)
{
    ...
    if(*first){
        ...
        *first = 0;
        ...
        //kernel
        fseek(img, (long)(*nbytes)+512, SEEK_SET);
        fwrite(seg, (long)(size), 1, img);
        *nbytes += size;
    }
