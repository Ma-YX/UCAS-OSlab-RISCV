...
void do_scheduler(void)
{
    ...
    if(current_running->pid > 1 && current_running->status == TASK_BLOCKED){
        prints("cur_pid: %d, state: %d\n\r", current_running->pid, current_running->status);
    }
    ...
    if(last_running->pid > 1 && current_running->pid == 1 && last_running->status == TASK_BLOCKED){
    //    prints("return to shell\n\r");
    }
    ...
    //sbi_console_putstr("\n\nswitch_to\n");
    if(last_running->pid != next_running->pid)
    //prints("%d,%d ", last_running->pid, next_running->pid);
...
void do_block(list_node_t *pcb_node, list_head *queue)
{
    ...
    current_running->status = TASK_BLOCKED;
    //prints("blocked pid: %d %d %d %d %d %d\n\r", current_running->pid, current_running->save_sp, current_running->user_sp, current_running->kernel_sp,
    //                                             current_running->pgdir, current_running->pa_table);
...
void do_unblock(list_head *queue)
{
    ...
    //prints("unblocked pid: %d %d %d %d %d %d\n\r", unblocked_task->pid, unblocked_task->save_sp, unblocked_task->user_sp, unblocked_task->kernel_sp,
    //                                                     unblocked_task->pgdir, unblocked_task->pa_table);
